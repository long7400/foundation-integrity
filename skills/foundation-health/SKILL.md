...
5. **Structural drift (if tooling exists).** If the repo has dependency-graph / duplication / complexity tools wired in, read their trend, not just their current pass/fail. Rising duplication or a new dependency cycle is the machine-measured version of the same story. If no such tooling exists, note that as a gap — don't install it here.
   ...
   The concrete git-only signals to trend live in [`templates/fitness/git-only.md`](../../templates/fitness/git-only.md): cross-boundary change-coupling, churn hotspots, blast radius, workaround-marker density. Compute them from `git log` alone — no stack tooling needed.
...
- Don't install new tooling as a side effect. If a structural signal is missing, name the gap and leave the decision to the maintainer (or to `setup-foundation-integrity`).
