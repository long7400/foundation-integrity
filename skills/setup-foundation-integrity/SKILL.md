...
- **`CLAUDE.md` and `AGENTS.md` at the repo root** — which exists? Is there already an `## Foundation Integrity` or `## Personal Operating Rules` block?
...
The foundation gate lives as a block in `CLAUDE.md` (Claude Code) or `AGENTS.md` (Codex). Pick the file already in use:

- If `CLAUDE.md` exists, edit it.
- Else if `AGENTS.md` exists, edit it.
- If neither exists, ask which to create — don't pick for them.
- Never create `AGENTS.md` when `CLAUDE.md` exists, or vice versa.

If a foundation block already exists, update it in place — don't append a duplicate, don't overwrite the user's surrounding edits.
...
- Insert `foundation-audit` into the seam between `to-spec` and `implement` / `tdd` / `prototype` — the gate runs after the spec is written, before code.
...
- The guard clears a surface change only when a **valid v1 receipt** (or an ADR embedding the same block) **in the same change set names the exact path**. Copy [`review-receipt.md`](../../templates/hooks/review-receipt.md) as the format, and create `.foundation/receipts/` if the repo will use receipts rather than full ADRs.
...
- Insert/update the block in the chosen file.
