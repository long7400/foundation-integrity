...
Model and effort choices live in those reviewed project envelope files, not in a
second matrix or a user-level profile copy.
...
The GLM profiles and their hash manifest are owned by the adopting project's pinned
loopback gateway lifecycle. `cliproxy-glm.sh setup` binds the project files and keeps
binary, config, state, and credentials under project `.foundation/`; `remove` deletes
only that project's gateway state. No primary-profile manager or global Codex path is
claimed.
...
operations require exact project-source bytes plus adoption-ledger provenance, and
their launch argv binds CLI overrides for the envelope above project config.
...
The two GLM auxiliaries are a bounded compatibility tier with project-local profile
ownership. The receipt-bound launcher may use them only after exact project
source/manifest attestation, gateway health, and a credentialed smoke. Herdr still owns creation,
